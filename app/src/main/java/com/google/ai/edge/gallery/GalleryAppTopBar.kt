...
fun GalleryTopAppBar(
  ...
  val blueEdgeGradient = Brush.linearGradient(colors = listOf(Color(0xFF85B1F8), Color(0xFF3174F1)))
    ...
    title = {
      Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Row(
          ...
          if (title.contains("Blue Edge") || title == stringResource(R.string.app_name)) {
            Icon(
              painterResource(R.drawable.view_in_ar_24px),
              modifier = Modifier.size(20.dp),
              contentDescription = null,
              tint = Color.Unspecified,
            )
          }
          val isAppName = title == stringResource(R.string.app_name)
          BasicText(
            text = title,
            maxLines = 1,
            color = { if (isAppName) Color.Unspecified else titleColor },
            style = if (isAppName) {
              MaterialTheme.typography.titleMedium.copy(
                fontWeight = androidx.compose.ui.text.font.FontWeight.Medium,
                brush = blueEdgeGradient,
              )
            } else {
