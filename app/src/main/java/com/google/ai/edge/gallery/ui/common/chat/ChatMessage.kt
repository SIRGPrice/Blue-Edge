...
/** Chat message combining text, images, and audio from the same prompt. */
class ChatMessageComposite(
  val content: String,
  ...
  ChatMessage(
    type = ChatMessageType.COMPOSITE,
    side = side,
    latencyMs = latencyMs,
    accelerator = accelerator,
    hideSenderLabel = hideSenderLabel,
  ) {
  override fun clone(): ChatMessageComposite {
    return ChatMessageComposite(
      content = content,
      bitmaps = bitmaps.toList(),
      imageBitMaps = imageBitMaps.toList(),
