...
private const val LAST_LOADED_MODEL_KEY = "last_loaded_model_name"
...
constructor(
  ...
  fun selectModel(model: Model) {
    if (_uiState.value.selectedModel.name != model.name) {
      ...
      // Persist the last loaded model name so it can be restored on next launch.
      dataStoreRepository.saveSecret(LAST_LOADED_MODEL_KEY, model.name)
    }
  }

  /** Returns the name of the last model that was loaded into memory, or null if none saved. */
  fun getLastLoadedModelName(): String? {
    return dataStoreRepository.readSecret(LAST_LOADED_MODEL_KEY)
  }
