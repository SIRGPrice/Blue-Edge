 ...
 *
 * Android `actual` for `MarkdownText`. Mirrors the original implementation
 * in `:app/.../ui/common/MarkdownText.kt` so behaviour matches once the
 * shared composable is consumed from `:app`.
 */
...
actual fun MarkdownText(
  ...
  val resolvedLink = defaultMarkdownLinkColor(linkColor)
  val fontSize =
      ...
      RichText(
        ...
        style = RichTextStyle(
          ...
          stringStyle = RichTextStringStyle(
            linkStyle = TextLinkStyles(style = SpanStyle(color = resolvedLink)),
          ),
        ),
      ) {
